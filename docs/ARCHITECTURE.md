...
- **Pipeline de Solicitudes**:
  1. **Autenticación**: Validación de identificadores y roles de usuario (`x-user-id`, `x-user-plan`).
  2. **Validación Zod**: Sanitización y tipado estricto de cada parámetro de entrada para neutralizar inyecciones de código.
  3. **Rate Limiter Dinámico**: Aplicación de límites por nivel de suscripción (Free: 10/h, Pro: 100/h, Max: 1,000/h).
  4. **Orquestador Multi-Modelo Resiliente**:
...
1. **Firebase Authentication**: Gestión de sesiones, identidades y tokens federados con altos estándares de seguridad.
2. **Supabase / PostgreSQL**: Registro de usuarios, balances de cuota y auditoría de transacciones criptográficas.
3. **Almacenamiento Local Cifrado**: Cada usuario mantiene un respaldo de sus chats directamente en su cliente, accesible sin conexión.
...
[⬅️ Anterior: Especificación de Modelos](./MODELS.md) • [Siguiente: Documentación de la API ➔](./API.md)
